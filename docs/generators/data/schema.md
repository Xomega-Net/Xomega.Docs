...
It also leverages the structure of sub-objects, as well as the way key fields or fieldsets are set up, in order to infer the foreign key relationships between the tables, so that they wouldn't need to be defined explicitly.
...
If the object contains a fieldset in the list of fields, then the table will contain all fields from that fieldset as columns, using its fields' names prefixed with the name of the fieldset in the object, if set.
...
Primary key is derived from the object's key, whether or not the column is required comes from the `required` attribute on the field, and the maximum length of the character columns is determined based on the `size` attribute of the field's type or its base types.
...
You can also specify attributes of the foreign keys for the field in the `sql:foreign-key` config element, such as the foreign key name, as well as the `delete` and `update` actions. For the implicit foreign key to the parent object, this is specified in the `sql:parent-foreign-key` element under the `sql:table` element.
...
The generator doesn't provide any additional customizations beyond what you can configure in the model or in the generator parameters.
