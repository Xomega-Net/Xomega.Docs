...
```xml title="Service Model Design.xgen"
<XomGeneratorConfig xmlns="http://schemas.xomega.net/v10/xgen"
  ...
  <doc:Output Path="../Docs/ServiceModelDesign.docx"/>
  
  <doc:Document Template=".Generators/Documentation/Templates/ServiceModel.docx"
                Title="MySolution's Service Model"
                Subject="Technical design for the MySolution's service model"
                Creator="[User]"
                Company="[Company]"/>
...
|**doc:Output**|
|Path|../Docs/ServiceModelDesign.docx|Relative path where to output the generated document.|
|**doc:Document**|
|Template|.Generators/Documentation /Templates/ServiceModel.docx|Path to the MS Word document that will be used as a template for the generated document. The path is relative to the model project.|
