...
After you create a solution using the Xomega solution wizard, you will see your selected projects in the *Solution Explorer*.
...
This project contains *Entity Framework*'s domain entities for your solution with the configuration of the `DbContext`, and provides an implementation of the business services defined in the `.Services.Common` project using those entities.

:::info
This project has **no dependencies** on any server-side frameworks, such as ASP.NET Core, so it could be (re)used from any server-side or client-side frameworks.
:::
...
- **Services** contains an implementation of the business services.
...
Otherwise, it will generate `.svc` service host files for the Xomega business services defined in the model.
...
:::info
This project should have **no dependencies** on any specific UI framework, such as Blazor, to allow sharing it with other C# clients and support an easy migration of your app to any future UI technologies, such as MAUI.
:::
...
- `Resources.resx` contains any common custom resources that may override the generated ones or provide additional resources, such as custom validation messages.
...
This project contains the code for the legacy ASP.NET WebForms web client that targets .NET Framework. The root folder contains the main application master page in the `Site.Master` file and other application files in sub-folders, as follows.

- **App_Start** folder contains the DI-enabled application initialization code for Owin.
...
The root folder contains the main application initialization code and markup, as well as the following sub-folders.

- **App_Start** contains the app startup and configuration code.
- **CacheLoaders** contains XomegaJS lookup cache loaders for loading contextual lookup data from the service operations to be used for lookup and validation.
- **Content** folder contains web application resources, such as CSS and images.
