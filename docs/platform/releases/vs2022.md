...
- **Panels and tabs layout for details Blazor views.**
  - Support for defining [named field groups](../../modeling/presentation#ui-field-groups) for UI panels or tabs.
  - Configuration of [panels and tabs](../../modeling/presentation#panel-and-tab-layout) to lay out data object field groups and child objects.
  - Ability to specify a [field group for displaying links](../../modeling/presentation#object-links-to-views).
  - Configuration of [localizable titles for panels](../../modeling/presentation#panel-layout) and [tabs](../../modeling/presentation#tabs-layout).
- **UI security enhancements.**
  - Configuration of [panels and tabs with conditional visibility](../../modeling/presentation#conditional-panel-visibility).
  - Authorization policy specification for [main menu links](../../modeling/presentation#security-policy-for-main-menu-links).
  - Support for [conditional authorization policies](../../modeling/presentation#conditional-security-policy) in Blazor views.
...
- **New and redesigned criteria framework.**
  - Simplified defining [search criteria in the model](../../modeling/services#filter-criteria) and [services](../../framework/services/querying#criteria-dto) to provide just the field without operators, etc.
...
- **Support for server-side paging and sorting.**
  ...
  - Added [paging mode configuration in the model](../../modeling/services#paging-configuration).
- **Support for querying large data sets.**
  ...
  - Allowed configuring the maximum number of records returned by operation [in the model](../../modeling/services#configuring-result-limiting) or in the custom code.
...
- **Model enhancements to control main menu items.**
  - Added ability to define [main menu links](../../modeling/presentation#main-menu-links) in the model on top-level views, including [configuring auto-search](../../modeling/presentation#opening-views-with-auto-search) and other initialization parameters.
...
- **[Standard VS connection dialog](../../visual-studio/model-project/configuring-generators#configuring-connection-info)** is now used instead of the custom OLE DB connection dialog.
  ...
  - [Resetting connection info](../../visual-studio/model-project/configuring-generators#editingresetting-connection-info) is now done via a selection in a new `DB Provider` property.
...
- **Support for [list types](../../modeling/types#list-types)**, backed by multi-database support of arrays in EF Core 8.x.
...
- Configuration of cascade delete/update actions now includes the [`restrict` value](../../modeling/domain#foreign-key-configuration).
...
### Minor changes
- Added ability to [specify triggers](../../modeling/domain#trigger-configuration) under a `sql:table` config element in the model.
...
- **Xomega model now allows you to specify an [authorization policy](../../modeling/presentation#security-policy) for each view.**
...
- Added ability to mark an entire service as [not exposed for WCF](../../modeling/services#wcf-configuration) (e.g. login), updated WCF generators accordingly.
- Generators for WPF, SPA and WebForms views now use the [`field-cols`](../../modeling/presentation#panel-layout) config for the number of columns, rather than the legacy [`ui:layout`](../../modeling/presentation#legacy-layout-config) config.
...
- Added support for [localization of static enumerations](../../modeling/static-data#localizing-static-items).
- Added support for [localization of dynamic enumerations](../../modeling/static-data#localizing-dynamic-items).
- Added model support for [custom enumeration names](../../modeling/types#custom-enumeration) that come from DB.
